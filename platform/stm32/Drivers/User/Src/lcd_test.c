/***
   ...
   *  @description
	*
	*	实验平台：反客STM32H750XBH6核心板 （型号：FK750M5-XBH6）+ 4.3寸RGB液晶屏(屏幕型号RGB043M1-480*272)
...
// layer1 显示内容 >>>>>
	...
	LCD_SetColor(LCD_WHITE);						 // 画笔不透明 
	LCD_DisplayText(110,140,"STM32H750XBH6 LTDC, ARGB4444, layer1"); 
	HAL_Delay(1000);
	
	LCD_SetBackColor(LCD_BLACK & 0x80FFFFFF);  // 设置背景色透明度，高8位的数字越小透明度越高，ARGB4444支持4位透明色，有16种透明状态
	LCD_SetColor(LCD_WHITE);		 
	LCD_DisplayText(110,200,"STM32H750XBH6 LTDC, ARGB4444, layer1"); 
	HAL_Delay(1000);
		
	LCD_SetBackColor(LCD_BLACK & 0x30FFFFFF );  // 设置背景色透明度，高8位的数字越小透明度越高，ARGB4444支持4位透明色，有16种透明状态
	LCD_SetColor(LCD_WHITE);
	LCD_DisplayText(110,260,"STM32H750XBH6 LTDC, ARGB4444, layer1"); 	
	HAL_Delay(1000);
	
	LCD_SetBackColor(LCD_BLACK  );  				 // 将背景色设置为 不透明
	LCD_SetColor(LCD_WHITE & 0x00FFFFFF);		 // 将画笔色设置为 完全透明	 
	LCD_DisplayText(110,320,"STM32H750XBH6 LTDC, ARGB4444, layer1"); 	
	HAL_Delay(2000);	
	...
	LCD_SetColor(LCD_WHITE);						// 画笔不透明 
	LCD_DisplayText(110,140,"STM32H750XBH6 LTDC, ARGB1555, layer1"); 
	HAL_Delay(1000);
	
	LCD_SetBackColor(LCD_BLACK & 0x00FFFFFF);  // 设置背景色透明度，高8位的数字越小透明度越高，ARGB1555仅支持一位透明色，即仅有透明和不透明两种状态
	LCD_SetColor(LCD_WHITE);	  					 // 画笔不透明 
	LCD_DisplayText(110,200,"STM32H750XBH6 LTDC, ARGB1555, layer1"); 
	HAL_Delay(1000);
		
	LCD_SetBackColor(LCD_BLACK  & 0xffFFFFF);  // 将背景色设置为 不透明
	LCD_SetColor(LCD_WHITE & 0x00FFFFFF);		 // 将画笔色设置为 完全透明	 
	LCD_DisplayText(110,260,"STM32H750XBH6 LTDC, ARGB1555, layer1"); 	
	HAL_Delay(2000);	
...
// 刷屏测试>>>>>
	...
	for(i=0;i<8;i++)
	{
		...
		LCD_DisplayText(112,134,"核心板型号：FK750M5");
...
// 颜色测试>>>>>
	...
	LCD_DisplayText(42,70,"核心板型号：FK750M5");
...
// 绘制其它内容>>>>>>>		
	...
	LCD_DisplayText(40,500+40,"核心板型号：FK750M5");	
